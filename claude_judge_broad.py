...
def judge_fit_broad(title: str, description: str, company_name: str, candidate_profile: str) -> dict:
    ...
    system = (
        ...
        "a people-management role -- but the postings you're evaluating here will NEVER have 'Senior' "
        "in front of a compound Manager title (e.g. 'Senior Product Marketing Manager'), since the "
        "keyword filter upstream of this judge already hard-excludes any title containing both "
        "'senior' and 'manager', regardless of order -- that's deliberate, the candidate has decided "
        "they're not qualified for that combination even when the compound function would otherwise "
        "be a fine IC title. Only treat 'Manager' as a "
